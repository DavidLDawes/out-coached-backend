...
/**
 * DESIGN.md §6 settlement sequencing. Triggered the moment a play's result
 * is newly recorded (operator's enterResult write — state stays "locked",
 * `result` appears for the first time). Deliberately guards on "result just
 * appeared" rather than "state == locked" alone, so this function's own
 * writes (settling -> settled) and undo's writes (clearing result) don't
 * cause it to recurse into itself.
 */
 ...
 * nothing to money-move here), but the game still needs to advance to the
 * next play, which only a function can do once phase 1's client-side
 * bootstrap path is removed.
 */
...
/**
 * DESIGN.md §3/§7.2: reverses the most recent settled play only — an
 * operator-initiated action (not a passive trigger), since it needs a role
 * check and an explicit "which play" target. Reverses via the ledger (§6),
 * not by recomputing settlement math, so the reversal amount is exactly
 * what was actually applied regardless of any later code changes. Rewinds
 * `currentPlayId` back to the corrected play so the operator's normal
 * "enter result" flow (which always targets currentPlayId) can re-enter it.
 */
